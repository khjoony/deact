...
def KospiInsert(filename):
    ...
    for i in range(0, len(g_data)):
        ...
        Kospi.objects.create(name=g_data[i][0], code=g_data[i][1], feature=g_data[i][2])
...
def KosdakInsert(filename):
    g_path = get_path(filename)
    g_data = save_dict_data(g_path)

    for i in range(0, len(g_data)):
        print(g_data[i][0], g_data[i][1])
        Kosdak.objects.create(name=g_data[i][0], code=g_data[i][1], feature=g_data[i][2])
